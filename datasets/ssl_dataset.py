...
class SSL_Dataset:
    ...
    def get_data(self):
        ...
        if self.train:
            ...
            calib_size = len(dset) - train_size
            train_data, calib_data = torch.utils.data.random_split(dset, [train_size, calib_size])

            return (train_data.dataset.data,
                    train_data.dataset.targets,
                    calib_data.dataset.data,
                    calib_data.dataset.targets)
        else:
    ...
    def get_ssl_dset(self, num_labels, index=None, include_lb_to_ulb=True,
        ...
        train_data, train_targets, calib_data, calib_targets = self.get_data()
        num_classes = self.num_classes
        transform = self.transform
        data_dir = self.data_dir

        lb_data, lb_targets, ulb_data, ulb_targets = split_ssl_data(train_data, train_targets, 
        ...
        calib_dset = BasicDataset(calib_data, calib_targets, num_classes, 
                               transform, False, None, onehot)
        
        return lb_dset, ulb_dset, calib_dset
